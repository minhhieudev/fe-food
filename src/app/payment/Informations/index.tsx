  ...
  return (
    <div className="bg-white rounded-2xl shadow-md overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {dataMoney?.map((item: any, index: number) => (
            <div key={index} className={`relative rounded-xl overflow-hidden ${
              index === 0 ? "bg-gradient-to-r from-amber-500 to-orange-500" : "bg-gradient-to-r from-emerald-500 to-teal-500"
            }`}>
                  ...
                  <div>
                    ...
                    <div className="flex items-end gap-1">
                      <p className="text-2xl font-bold">
                        {formatPriceVND(Number((Number(item?.buget) * Number(currency.exchangeRate)).toFixed(2)))}
                      </p>
      ...
      <div className="bg-gray-50 p-6">
        <h3 className="text-lg font-bold text-gray-700 mb-4">{lang.gen("recharge.promotions")}</h3>
            ...
            if(item.key == 'depositDiscount') {
              return item.value.map((e: any, idx: number) => (
                <div key={idx} className="group relative rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-all">
                  <div className="absolute -right-10 -top-10 w-20 h-20 bg-orange-500 rotate-45 transform group-hover:scale-110 transition-transform"></div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-3 border-t border-dashed border-gray-200 pt-3">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="text-lg font-bold text-orange-500">+{formatPriceVND(Number(e.amount) * (e.discountPercent/100))}</div>
                      </div>
                    </div>
                  </div>
                </div>
